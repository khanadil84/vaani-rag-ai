...
import { INDEXING_PIPELINE } from '../lib/knowledge/config'
...
export default function IndexingPipeline({ className }: IndexingPipelineProps) {
  return (
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[13px] text-slate-400">
            From raw dataset to searchable vector index.
          </p>
        </div>
        <NeutralStatusBadge label="Not started" />
      </div>

      <ol className="mx-auto max-w-3xl">
        {INDEXING_PIPELINE.map((stage, index) => {
          const Icon = stage.icon
          const isLast = index === INDEXING_PIPELINE.length - 1
          return (
            <Fragment key={stage.id}>
              ...
              {!isLast && (
                ...
                >
                  <span
                    className="absolute top-0 left-1/2 size-1 -translate-x-1/2 rounded-full bg-slate-500/70 animate-flow-y"
